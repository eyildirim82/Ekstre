...
function generateExcel(data: any[] | { sheetName: string; data: any[][] }[], filename: string): Buffer {
...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    if (from || to) {
      dateFilter.txnDate = {};
      if (from) dateFilter.txnDate.gte = new Date(`${from}T00:00:00`);
      if (to) dateFilter.txnDate.lte = new Date(`${to}T23:59:59`);
    }
    ...
    const customerBalances = customerStats.map(customer => {
      ...
      return {
        ...
        code: customer.code,
        name: customer.name,
        balance: centsToTL(balanceCents),
        balanceCents,
    ...
    const monthlyData = monthlyTrend
      ...
      .map(item => ({
        date: item.txnDate,
        debit: centsToTL(Number(item._sum?.debitCents ?? 0)),
        credit: centsToTL(Number(item._sum?.creditCents ?? 0)),
        count: item._count
      }));
    ...
    const report = {
      ...
      summary: {
        totalCustomers,
        totalTransactions,
        totalDebit: centsToTL(totalDebitCents),
        totalCredit: centsToTL(totalCreditCents),
        netAmount: centsToTL(totalDebitCents - totalCreditCents)
      },
      customerAnalysis: {
        totalCustomers,
        ...
        totalDebtAmount: centsToTL(debtCustomers.reduce((sum, c) => sum + c.balanceCents, 0)),
        totalCreditAmount: centsToTL(creditCustomers.reduce((sum, c) => sum + c.balanceCents, 0))
      },
      topCustomers: {
      ...
      docTypeBreakdown: docTypeStats.map((stat: any) => ({
        docType: stat.docType || 'Belirsiz',
        count: stat._count,
        totalDebit: centsToTL(Number(stat._sum?.debitCents ?? 0)),
        totalCredit: centsToTL(Number(stat._sum?.creditCents ?? 0)),
        netAmount: centsToTL(Number(stat._sum?.debitCents ?? 0) - Number(stat._sum?.creditCents ?? 0))
      })),
...
router.get('/aging-analysis', async (req: Request, res: Response) => {
  try {
    const { customerCode, format = 'json' } = req.query as any;
...
router.get('/trend-analysis', async (req: Request, res: Response) => {
  try {
    const { from, to, customerCode, groupBy = 'month' } = req.query as any;
    ...
    const trendArray = Object.values(trendData).map((item: any) => ({
      period: item.period,
      count: item.count,
...
router.get('/comparison', async (req: Request, res: Response) => {
  try {
    ...
    const customerFilter: any = {};
    if (customerCodes) {
      const codes = String(customerCodes).split(',').map((c: string) => c.trim());
    ...
    const comparison = period1Data.map(customer1 => {
      ...
      return {
        ...
        name: customer1.name,
        period1: {
          debit: centsToTL(p1Debit),
          credit: centsToTL(p1Credit),
          net: centsToTL(p1Net),
          transactionCount: customer1.transactions.length
        },
        period2: {
          debit: centsToTL(p2Debit),
          credit: centsToTL(p2Credit),
          net: centsToTL(p2Net),
          transactionCount: customer2 ? customer2.transactions.length : 0
        },
        change: {
          amount: centsToTL(change),
...
// Müşteri özet raporu
router.get('/customer-summary/:code', async (req: Request, res: Response) => {
  try {
    ...
    const report = {
      ...
      recentTransactions: transactions.map((t: any) => ({
        ...
        amount: centsToTL(t.debitCents > 0 ? t.debitCents : t.creditCents)
      })),
      docTypeBreakdown: docTypeStats.map((stat: any) => ({
        docType: stat.docType || 'Belirsiz',
        count: stat._count,
      ...
      monthlyTrend: monthlyTrend
        .sort((a: any, b: any) => new Date(a.txnDate).getTime() - new Date(b.txnDate).getTime())
        .map((item: any) => ({
          date: item.txnDate,
          debit: centsToTL(Number(item._sum?.debitCents ?? 0)),
          credit: centsToTL(Number(item._sum?.creditCents ?? 0)),
          net: centsToTL(Number(item._sum?.debitCents ?? 0) - Number(item._sum?.creditCents ?? 0))
        })),
      generatedAt: new Date().toISOString()
    };
...
router.get('/customer-list', async (req: Request, res: Response) => {
  try {
    ...
    // Müşteri filtresi
    const customerFilter: any = {};
    if (accountType) customerFilter.accountType = String(accountType);
    ...
    const report = {
      summary: {
        ...
        totalBalance: fmtTL(totalBalance),
        totalDebit: fmtTL(totalDebit),
        totalCredit: fmtTL(totalCredit),
        debtCustomers,
        creditCustomers,
        zeroCustomers
      },
...
router.get('/customers', async (req: Request, res: Response) => {
  try {
    const { search, accountType, balanceType } = req.query as any;
...
router.get('/customer-detail/:code', async (req: Request, res: Response) => {
  try {
    const { code } = req.params;
    ...
    // İstatistikler
    const totalDebitCents = transactions.reduce((sum, t: any) => sum + Number(t.debitCents), 0);
    const totalCreditCents = transactions.reduce((sum, t: any) => sum + Number(t.creditCents), 0);
    const balanceCents = totalDebitCents - totalCreditCents;

    // Belge türü analizi
    const docTypeAnalysis = transactions.reduce((acc, t: any) => {
    ...
    // Aylık özet
    const monthlySummary = transactions.reduce((acc, t: any) => {
    ...
    const report = {
      ...
      docTypeAnalysis: Object.entries(docTypeAnalysis).map(([docType, data]: [string, any]) => ({
        docType,
        count: data.count,
        totalDebit: centsToTL(data.totalDebit),
        totalCredit: centsToTL(data.totalCredit),
        netAmount: centsToTL(data.totalDebit - data.totalCredit)
      })),
...
router.get('/collections-summary', async (req: Request, res: Response) => {
  try {
    const { from, to, customerCode, format = 'json' } = req.query as any;
    ...
    const overdueDetails = overdueTransactions.map(tx => ({
      ...
      amount: centsToTL(tx.debitCents),
    ...
    const overdueCustomerSummary = Object.values(overdueByCustomer)
      .map((customer: any) => ({
        ...customer,
        totalAmount: centsToTL(customer.totalAmount * 100),
    ...
    const report = {
      ...
      summary: {
        totalReceivables: centsToTL(totalReceivables * 100),
        totalCollections: centsToTL(totalCollections * 100),
        collectionRate: Math.round(collectionRate * 100) / 100,
        overdueAmount: centsToTL(overdueAmount * 100),
        overdueTransactionCount: overdueTransactions.length,
        overdueCustomerCount: Object.keys(overdueByCustomer).length
      },
      expectedCollections: {
        days0to30: centsToTL(expectedCollections.days0to30 * 100),
        days31to60: centsToTL(expectedCollections.days31to60 * 100),
        days60plus: centsToTL(expectedCollections.days60plus * 100),
        total: centsToTL((expectedCollections.days0to30 + expectedCollections.days31to60 + expectedCollections.days60plus) * 100)
      },
      monthlyCollections: monthlyData.map(month => ({
        ...month,
        amount: centsToTL(month.amount * 100)
      })),
      overdueAnalysis: {
        summary: {
          totalAmount: centsToTL(overdueAmount * 100),
      ...
      const excelData = [
        ...
        // Gecikmiş müşteriler sayfası
        {
          ...
          data: [
            ...
            ...overdueCustomerSummary.map((customer: any) => [
